...
def process_hdf5_dataset(vla_dataset):
    """计算数据集的统计信息，用于训练时的数据标准化"""
    
    EPS = 1e-8  # 数值稳定性阈值
    episode_cnt = 0
    state_sum = 0        # 状态总和
    state_sum_sq = 0     # 状态平方和
    z_state_sum = 0      # 零化后状态总和
    z_state_sum_sq = 0   # 零化后状态平方和
    state_cnt = 0        # 状态样本总数
    nz_state_cnt = None  # 非零状态计数
    state_max = None     # 状态最大值
    state_min = None     # 状态最小值
    
    # 遍历所有episode计算统计信息
    for i in tqdm(range(len(vla_dataset))):
        episode = vla_dataset.get_item(i, state_only=True)  # 获取完整轨迹
        episode_cnt += 1
    ...
    result = {
        ...
        "state_mean": (state_sum / state_cnt).tolist(),           # 均值
        "state_std": np.sqrt(                                     # 标准差（考虑零值）
            np.maximum(
                (z_state_sum_sq / nz_state_cnt) - (z_state_sum / state_cnt)**2 * (state_cnt / nz_state_cnt),
                np.zeros_like(state_sum_sq),
            )
        ).tolist(),
        "state_min": state_min.tolist(),                          # 最小值
        "state_max": state_max.tolist(),                          # 最大值
    }
    return result
